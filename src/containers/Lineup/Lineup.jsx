...
import { capture, save, load, share, loadSharedLineup, generatePlayers, getFormation } from 'utils';
...
const ShareMessage = ({ link, intl }) => (
  <div className="share-message">
    <h3 className="share-message__header">
      <FormattedMessage id="lineup.sharemessage.heading" defaultMessage="Link to Share" />
    </h3>
    ...
    <span>
      ...
      >
        <FormattedMessage id="lineup.sharemessage.clipboard" defaultMessage="Copy to Clipboard" />{' '}
        <img
          src={clipIcon}
          width="13"
...
const Tactic = ({ intl }) => {
  ...
  function editPlayer(id, nName, nNum, nPosition, nGoal, nYellowCard, nRedCard) {
    setPlayers(
      players.map((player) => {
        if (player.id === id) {
          return {
    ...
    <div className="tactic">
      <Container>
        ...
        <h1 className="heading">
          <FormattedMessage id="lineup.heading" defaultMessage="Lineup Builder" />
        </h1>
          ...
          >
            <div className="form-group">
              <label className="label" htmlFor="tactic-name">
                <FormattedMessage id="lineup.options.name" defaultMessage="Lineup Name" />
              </label>
              <input className="input form-control" id="tactic-name" name="tactic-name" type="text" value={name} onChange={(event) => setName(event.target.value)} />
            </div>
            <div className="form-group">
              <label className="label" htmlFor="Ground Type">
                <FormattedMessage id="lineup.options.groundtype" defaultMessage="Ground Type" />
              </label>
              <select onChange={(event) => setFieldType(event?.target.value)} value={fieldType} className="form-control">
                <option value="grass">
                  {intl.formatMessage({
            ...
            <div className="form-group">
              <label className="label" htmlFor="Formation">
                <FormattedMessage id="lineup.options.Formation" defaultMessage="Formation" />
              </label>
              <select onChange={(event) => changeFormation(event?.target.value)} value={formation} className="form-control">
            ...
            <div className="color-options">
              <div className="color-option">
                <span className="tactic__label">
                  <FormattedMessage id="lineup.options.color.primary" defaultMessage="Primary" />
                </span>
                <ColorPicker color={mainColor} setColor={setMainColor} />
              </div>
              <div className="color-option">
                <span className="tactic__label">
                  <FormattedMessage id="lineup.options.color.secondary" defaultMessage="Secondary" />
                </span>
                <ColorPicker color={secondaryColor} setColor={setSecondaryColor} />
              </div>
              <div className="color-option">
                <span className="tactic__label">
                  <FormattedMessage id="lineup.options.color.number" defaultMessage="Number" />
                </span>
